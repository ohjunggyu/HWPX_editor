...
async def send_to_ollama(messages: list) -> str:
    ...
    cleaned_messages = []
    for m in messages:
        cleaned_messages.append({"role": m["role"], "content": strict_clean(m["content"])})

    payload = {"model": MODEL_NAME, "messages": cleaned_messages, "stream": False}
    ...
    async with httpx.AsyncClient() as client:
        with open(file_path, "rb") as f:
            ...
            try:
                ...
                print(
                    "   문서를 파싱하려면 먼저 터미널을 열고 다음 명령어를 실행해 서버를 켜주세요:"
                )
...
async def main():
    ...
    if os.path.exists(summary_path):
        ...
        print(
            "🔍 각 양식의 내용을 최초 분석하여 요약 캐시(summary.json)를 생성하는 중입니다. (최초 1회, 일시적으로 시간이 소요됨)"
        )
        template_summaries = {}
...
만약 사용자의 요청이 모호하거나, 어떤 양식을 사용해야 할지 판단하기 어려운 경우, 사용자에게 필요한 정보(예: 날짜, 이름, 소속, 목적 등)를 친절하게 질문하여 정보를 수집하세요.

만약 사용자가 제공한 정보가 특정 하나의 문서를 고르기 모호하다면, 적합하다고 판단되는 문서들의 파일명들을 출력하고, 확인절차를 거친 후 추가 정보를 요청하세요.
"""
    ...
    if not selected_template:
        print(
            f"⚠️ LLM이 정확한 양식을 찾지 못했습니다. (응답: {selected_filename}) 기본 양식을 사용합니다."
        )
    ...
    if DEBUG_MODE:
        ...
        save_debug_log(
            "document_analysis", f"full_document_structure_{timestamp}.json", full_blocks_json
        )
        print(
    ...
    blocks_json = json.dumps(
        blocks[:120], ensure_ascii=False, indent=2
    )  # Send up to 120 blocks context
...
2. 정보가 부족하다면, 사용자에게 친절하게 질문하여 정보를 수집하십시오. 여러번 질문해야 할 수도 있습니다.
...
5. 사용자가 제공하는 정보가 완성된 형태가 아닐 수 있습니다. 이때는 사용자가 제공한 정보를 바탕으로 완성된 형태의 문서를 생성해야 합니다.
6. 당신은 군용 비서이므로, 군대에서 사용하는 용어를 사용해야 합니다.
7. 제공되지 않은 정보는 임의로 생성하지 말고, 사용자에게 질문하여 정보를 수집하십시오.
8. 전체 구조를 고려하여 보기 좋게 수정하십시오.
        ...
        if '"block_id"' in llm_response and "[" in llm_response:
            if DEBUG_MODE:
                ...
                save_debug_log(
                    ...
                    json.dumps(history_copy, ensure_ascii=False, indent=2),
                )
            ...
            try:
                ...
                if DEBUG_MODE:
                    ...
                    save_debug_log(
                        "api_requests",
                        f"modify_payload_{timestamp}.json",
